...
5. **Unknown means unknown.** Expired occupied cells lose observed-free status and retain an unresolved-hazard buffer until observed clear. Steep cells receive the same footprint inflation as obstacles. Verified line-of-sight shortcuts smooth grid turns without crossing unknown support. A* cannot pass through its own inflation margin or secretly substitute a different destination. Clearing occupancy requires three consecutive supporting frames.
