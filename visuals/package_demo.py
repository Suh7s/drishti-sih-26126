...
config=json.loads((a.run/'config.json').read_text()) if (a.run/'config.json').exists() else {}
goal=np.array(config.get('goal',[8,0]));disaster=config.get('world')=='disaster'
scope='rolling disaster terrain' if disaster else 'flat static course'
...
d.text((1100,114),('02   RGB + LEARNED TERRAIN OVERLAY' if disaster else '02   LEFT CAMERA / RGB'),font=F[14],fill=MUTED)
d.text((1100,442),('03   STEREO DEPTH / 0–6 m' if disaster else '03   STEREO DEPTH / 0–5 m'),font=F[14],fill=MUTED)
...
d.text((32,881),f'Recorded closed-loop run · {scope} · known start pose · camera-driven navigation',font=F[12],fill=MUTED)
...
for idx,t in enumerate(samples):
    ...
    fields=[('STATE',row['state']),('SIMULATION',f'{t-start:05.1f} s'),('COMMAND',f"{row['command'][0]:.2f} m/s"),
            ('TRACKING',f"{row['inliers']} inliers"),('VISION LATENCY',f"{row['compute_ms']:.0f} ms"),('GOAL DISTANCE',f"{np.linalg.norm(np.array(row['pose'][:2])-goal):.2f} m")]
...
dr.text((80,160),('A destination reached. A run you can audit.' if score['success'] else 'Mission stopped. Evidence preserved.'),font=F[44],fill=INK)
dr.text((80,239),f'Measured in Webots · {scope}',font=F[25],fill=MUTED)
...
dr.text((80,705),('Synthetic terrain only. No real flood deployment or moving-obstacle validation.' if disaster else 'No hardware, learned semantics, dynamic obstacles or loop-closing SLAM validated.'),font=F[19],fill=MUTED)
